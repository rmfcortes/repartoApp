...
export class RutaPage {
  ...
  netSub() {
    if (this.netSubscription) {
      return;
    }
    this.netSubscription = this.netService.isConnected.subscribe(resp => {
  ...
  salir() {
    ...
    if (this.netSubscription) { this.netSubscription.unsubscribe(); }
  ...
  async doRefresh(event) {
    await this.pedidosEntrantes();
    await this.pedidosNoAsignados();
