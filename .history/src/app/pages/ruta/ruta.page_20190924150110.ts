...
export class RutaPage implements OnInit, OnDestroy {
  ...
  pinHappyMsg = '../../../assets/img/pinHappyMsg.svg';
  pinSadMsg = '../../../assets/img/pinSadMsg.svg';
  pinAngryMsg = '../../../assets/img/pinAngryMsg.svg';
  ...
  pedidosEntrantes() {
    this.notificationService.clientesObs.subscribe((clientes: Pedido[]) => {
      ...
      this.clientes.forEach(p => {
        const momento = ( p.pedido.createdAt + 1800000 - hora ) / 1000;
        console.log(momento);
  ...
  countDown() {
    setTimeout(() => {
      ...
      this.clientes.forEach(p => {
        if (p.minutos > 15 ) {
          if (p.msgPend) {
            p.pin = this.pinHappyMsg;
          } else {
            p.pin = this.pinHappy;
          }
        } else if (p.minutos > 5 ) {
          if (p.msgPend) {
            p.pin = this.pinSadMsg;
          } else {
            p.pin = this.pinSad;
          }
        } else {
          if (p.msgPend) {
            p.pin = this.pinAngryMsg;
          } else {
            p.pin = this.pinAngry;
          }
        }
