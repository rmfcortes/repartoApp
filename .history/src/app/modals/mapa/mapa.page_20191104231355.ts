...
export class MapaPage implements OnInit {
  ...
  async getClientesRegistrados() {
    ...
    if (this.clientesDesatendidos && this.clientesDesatendidos.length > 0) {
      ...
      this.clientesDesatendidos = this.clientesDesatendidos.filter(c => c.direccion);
      if (this.clientesDesatendidos.length > 0) {
  ...
  async sortClientesRegistrados() {
    ...
    for (const c of this.clientesDesatendidos) {
      ...
      this.pines.push(pin);
      c.distancia = await this.calculaDistancia(this.ubicacion.lat, this.ubicacion.lng, c.direccion.lat, c.direccion.lng);
  ...
  pinSelected(pin) {
    let cliente;
    if (this.origen === 'pedidos') {
      cliente = this.clientes.find(c => c.pedido.id === pin.id);
    }
    if (this.origen === 'registrados') {
