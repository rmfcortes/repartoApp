...
export class UbicacionService {
        ...
        .subscribe((location: BackgroundGeolocationResponse) => {
          this.ngZone.run(() => {
            if (location.accuracy > 35) {
              this.presentToast('Ubiación muy loca');
              return;
            }
            this.presentToast('Ubiación subida');
  ...
  async getLocation(): Promise<BackgroundGeolocationResponse> {
    return new Promise((resolve, reject) => {
      this.backgroundGeolocation.getCurrentLocation().then((location: BackgroundGeolocationResponse) => {
        this.presentToast('Current lat ' + location.latitude);
        this.updateLocation(location);
        resolve(location);
      });
    });
  ...
  checkStatus() {
    this.backgroundGeolocation.checkStatus().then((status: ServiceStatus) => {
      ...
      setTimeout(() => {
        this.presentToast('Activo?' + status.locationServicesEnabled);
      }, 3000);
      if (!status.isRunning) {
        this.backgroundGeolocation.start();
      }
