...
export class VentaService {
  ...
  setViajeDB( pass ) {
    return new Promise (async (resolve, reject) => {
      try {
        this.db.object(`carga/${this.uid}/venta/sumario`).set(this.viaje);
  ...
  async updateCargaDB(prods: ProductoCarga[]) {
    ...
    return new Promise(async (resolve, reject) => {
      for (const p of prods) {
        if (p.cantidad) {
          await this.db.object(`ventas/${this.uid}/${this.fecha}/carga/${this.viaje}/${p.id}`).update({actual : p.actual - p.cantidad});
  ...
  pushVenta(productos: ProductoCarga[], datos: DatosVenta, cuenta) {
    ...
    this.db.object(`ventas/${this.uid}/${this.fecha}/detalles/${this.viaje}/venta`).query.ref
