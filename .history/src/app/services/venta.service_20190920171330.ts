...
export class VentaService {
  ...
  getResumen(): Promise<ResumenViaje> {
    return new Promise(async (resolve, reject) => {
      ...
      // Reset viaje
      const resSub = this.db.object(`ventas/${this.uid}/${this.fecha}/detalles/1`).valueChanges()
  ...
  getVentas(batch, lastkey) {
    return new Promise((resolve, reject) => {
      if (lastkey) {
        const venSub = this.db.list(`ventas/${this.uid}/${this.fecha}/venta/1`, data =>
        ...
        return this.db.list(`ventas/${this.uid}/${this.fecha}/venta/1`, data =>
