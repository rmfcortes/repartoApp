...
export class AuthService {
  ...
  verificarUsuario( id ) {
    return new Promise ( (resolve, reject) => {
      const infSub = this.db.object(`repartidores/${id}`).valueChanges().subscribe(async (data: any) => {
          infSub.unsubscribe();
          if (data) {
            if (data.puesto === 'Vendedor' || data.puesto === 'Administrador') {
              if (data.status === 'Inactivo') {
                const resp = await this.hasVentas();
                if (resp) {
                  resolve(true);
                }
              } else {
  ...
  hasVentas() {
    return new Promise((resolve, reject) => {
      const dataSub = this.db.object(`ventas/${this.uid}/${this.fecha}`).valueChanges().subscribe(info => {
        dataSub.unsubscribe();
        if (info) {
          const error = {
            code: 'has-ventas'
