...
export class ClientesService {
  ...
  listenPedidos() {
    ...
    this.db.list(`pedidos/${this.uid}`).query.ref.on('child_added', snapshot => {
      this.ngZone.run(() => {
  ...
  pedidoQuitado() {
    this.db.list(`pedidos/${this.uid}`).query.ref.on('child_removed', snapshot => {
      this.ngZone.run(() => {
        const cliente = snapshot.val();
  ...
  mensajesEntrantes() {
    this.db.list(`pedidos/${this.uid}`).query.ref.on('child_changed', snapshot => {
      this.ngZone.run(() => {
        const cliente = snapshot.val();
