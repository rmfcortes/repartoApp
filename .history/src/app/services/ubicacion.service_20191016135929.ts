...
export class UbicacionService {
  ...
  async comparaLoc(position: Geoposition) {
    this.db.list(`test/accuray`).push(position.coords.accuracy);
    if (position.coords.accuracy > 25) {
      return;
    }
    this.db.list(`test/last`).push(this.lastLoc.lat);
    this.db.list(`test/last`).push(this.lastLoc.lng);
    this.db.list(`test/position`).push(position.coords.latitude);
    this.db.list(`test/position`).push(position.coords.longitude);
    if (!this.lastLoc.lat || !this.lastLoc.lng) {
      ...
      this.db.list(`test/distancia`).push(d);
      if (d < 5) {
        // Muy cerca
        return;
      } else {
